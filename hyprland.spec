...
# Metadata for version reporting (generated by create-srpm.sh)
Source99:       metadata.env
...
cmake -B build \
  ...
  -DGIT_COMMIT_HASH="${HASH:-unknown}" \
  -DGIT_COMMIT_DATE="${DATE:-unknown}" \
  -DGIT_BRANCH="${BRANCH:-unknown}" \
  -DGIT_COMMIT_MESSAGE="${MESSAGE:-unknown}" \
  -DGIT_DIRTY=OFF
cmake --build build --parallel %{_smp_build_ncpus}
